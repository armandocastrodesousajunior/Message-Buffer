...
export class WindowRepository {
  ...
  async deleteByBufferAndStatus(bufferId: string, statuses: WindowStatus[]): Promise<void> {
    await getDatabase()('windows').where({ buffer_id: bufferId }).whereIn('status', statuses).delete();
  }
}
